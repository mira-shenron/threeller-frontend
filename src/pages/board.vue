        ...
        <main class="board-container">
            <div class="board-header flex space-between">
                <div class="board-title-container flex align-center">
                    ...
                    <div class="board-menu-btn clickable invite-btn">
                        Invite
                    </div>
                ...
                <transition name="menu-icn-animation">
                    ...
                    >
                        ...
                        <div class="board-menu-text flex align-center">
                            Show Menu
                        </div>
                    </div>
                </transition>
...
export default {
    ...
    methods: {
        ...
        updateCardInBoard(card) {
            ...
            if (this.board.groups) {
                for (let i = 0; i < this.board.groups.length; i++) {
                    if (this.board.groups[i].cards) {
                        for (
                            let j = 0;
                            j < this.board.groups[i].cards.length;
                            j++
                        ) {
        ...
        copyList({ list, currId }) {
            const board = this.board;
            const listIdx = board.groups.findIndex(
                (list) => list.id === currId
            );
            if (listIdx < 0) return;
